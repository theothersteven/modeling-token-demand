...
(function (root, factory) {
  ...
  const INDUSTRY_PARAMETERS = [
    {key: 'lambda', symbol: 'λ', label: 'Capability horizon', min: 1e-6,
     help: 'Baseline scope the model can feasibly handle in q(s;m) = exp[−(s/(λm))^ν].'},
    {key: 'nu', symbol: 'ν', label: 'Capability shape', min: 1e-6,
     help: 'How sharply the feasible share falls as delegated scope grows.'},
    {key: 'a', symbol: 'a', label: 'Execution ease', min: 1e-6,
     help: 'Scales the execution horizon a·m·(ηx)^α in r(s,x;m,η).'},
    {key: 'alpha', symbol: 'α', label: 'Inference returns', min: 1e-6, max: 0.999999,
     help: 'Diminishing return from effective inference ηx; must be below one.'},
    {key: 'h0', symbol: 'h₀', label: 'Fixed review time', min: 0,
     help: 'Fixed hours per review in h(s) = h₀ + h₁[(1+s)^β − 1].'},
    {key: 'h1', symbol: 'h₁', label: 'Variable review scale', min: 1e-9,
     help: 'Scales the part of review time that grows with delegated scope.'},
    {key: 'beta', symbol: 'β', label: 'Review elasticity', min: 0,
     help: 'How quickly review time grows with scope (0: fixed, 1: proportional).'},
    {key: 'b', symbol: 'b', label: 'Value of successful work', min: 1e-9,
     help: 'Dollar value of one successfully completed work unit.'},
    {key: 'w', symbol: 'w', label: 'Value of human attention', min: 1e-9,
     help: 'Dollar value of one hour of human review.'},
    {key: 'mu', symbol: 'μ', label: 'Hurdle location', min: -Infinity,
     help: 'Location of the logistic adoption-hurdle distribution.'},
    {key: 'sigma', symbol: 'σ', label: 'Hurdle spread', min: 1e-9,
     help: 'Spread of adoption hurdles: small values make adoption threshold-like.'},
    {key: 'W', symbol: 'W', label: 'Potential work', min: 1e-9,
     help: 'Total work units available in the work-limited regime (Q = W·A).'},
    {key: 'H', symbol: 'H', label: 'Human attention', min: 1e-9,
     help: 'Review hours available in the attention-limited regime (Q = H·s/h(s)).'},
  ];
  ...
  const OUTPUTS = [
    ...
    {key: 'rho', label: 'Value of attention (shadow price), ρ*', unit: '$ per hour', log: true, value: o => o.rho},
